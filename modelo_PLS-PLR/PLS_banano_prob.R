...
#elimina aquellas que tiene error de máscara

datos1= filter(datos, datos$V7!=1) #elimina error en máscara #quedan 109
datos2=datos1
totalbuenas <- datos2 %>% group_by(V2) %>% summarize(n=n()) 
...
for (lambda in lseq){
 ...
 for(m in 1:nfila){# crea matriz de calibración
		...
		PLSbanano=PLSR1BinFit(Ycal, Xcal,S=2, penalization = lambda)
...
write.table((ProbY), file = "D:/ugarte/R/programas/ggbiplot/ggbiplotBI/ProbYcorr&masc.txt", sep=" ", append=FALSE, col.names=TRUE, row.names=FALSE, dec=",", quote= FALSE)
